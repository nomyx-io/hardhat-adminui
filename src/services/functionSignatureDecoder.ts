 ...
 */
const COMMON_FUNCTION_SIGNATURES: { [selector: string]: string } = {
 ...
 */
const COMMON_EVENT_SIGNATURES: { [hash: string]: string } = {
...
export class FunctionSignatureDecoder {
  ...
  private cache: Map<string, DecodedFunction> = new Map();
  private eventCache: Map<string, DecodedEvent> = new Map();
  ...
  async decodeFunction(
    ...
    let decoded: DecodedFunction | null = null;
   ...
   */
  private async decodeInputsFromSignature(data: string, signature: string): Promise<any[]> {
    try {
   ...
   */
  private async decodeEventInputsFromSignature(log: ethers.providers.Log, signature: string): Promise<any[]> {
    try {
      const iface = new ethers.utils.Interface([`event ${signature}`]);
