...
export function createTransactionsRoutes(hre: HardhatRuntimeEnvironment): Router {
  const router = Router();
   ...
   */
  router.get("/api/transactions/:hash", async (req: AppRequest, res) => {
   ...
   */
  router.get("/api/transactions", async (req: AppRequest, res) => {
    const { 
      limit = '50', 
      address, 
      contract,
      ...
      for (const hash of txHashes.slice(0, limitNum)) {
        try {
          ...
          if (tx && receipt) {
            ...
            if (from && tx.from.toLowerCase() !== (from as string).toLowerCase()) {
              continue;
            }
            if (to && tx.to?.toLowerCase() !== (to as string).toLowerCase()) {
              continue;
            }
