...
export function createDashboardRoutes(hre: HardhatRuntimeEnvironment): Router {
  const router = Router();
   ...
   */
  router.get("/api/dashboard/:network", async (req: AppRequest, res) => {
    ...
    try {
      ...
      const contractTypes = deployments.reduce((acc: Record<string, number>, dep) => {
      ...
      try {
        const totalWei = deployments.reduce((sum, dep) => {
          if (dep.receipt?.gasUsed && (dep.receipt as any)?.effectiveGasPrice) {
            const gasCost = ethers.BigNumber.from(dep.receipt.gasUsed)
              .mul(ethers.BigNumber.from((dep.receipt as any).effectiveGasPrice));
            return sum.add(gasCost);
          }
          return sum;
        }, ethers.BigNumber.from(0));
        
        totalEthSpent = ethers.utils.formatEther(totalWei);
      } catch (error) {
      ...
      try {
        ...
        totalEvents = deployments.reduce((sum, dep) => {
          const eventCount = dep.abi?.filter((item: any) => item.type === 'event').length || 0;
   ...
   */
  router.post("/api/verify-contracts/:network", async (req: AppRequest, res) => {
    const { network } = req.params;
