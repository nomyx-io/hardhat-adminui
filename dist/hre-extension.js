...
const functionSignatureDecoder_1 = require("./services/functionSignatureDecoder");
(0, config_1.extendEnvironment)((hre) => {
    ...
    const watcher = chokidar_1.default.watch(deploymentsPath, {
        persistent: true,
        ignoreInitial: true,
    });
    ...
    watcher.on('add', async (filePath) => {
        ...
        if (path_1.default.extname(filePath) === '.json' && !filePath.includes('.dbg.json')) {
            try {
    ...
    hre.adminUI = {
        ...
        getDeployment: async (contractName) => {
            try {
        ...
        getContract: async (contractName, signer) => {
            const deployment = await hre.deployments.get(contractName);
            if (!deployment)
                return null;
        ...
        verifyContract: async (contractName) => {
            const deployment = await hre.deployments.get(contractName);
            if (!deployment) {
        ...
        simulateTransaction: async (from, to, data, value) => {
        ...
        getRecentTransactions: async (contractAddress, limit = 10) => {
            try {
        ...
        getTransactionDetails: async (txHash) => {
            try {
                ...
                const gasPrice = transaction.gasPrice || receipt.effectiveGasPrice || ethers_1.ethers.BigNumber.from(0);
                ...
                if (transaction.data && transaction.data !== '0x' && transaction.to) {
                    try {
                        ...
                        const contractDeployment = Object.values(deployments).find(dep => dep.address.toLowerCase() === transaction.to.toLowerCase());
                        let contractInterface = null;
                        if (contractDeployment && contractDeployment.abi) {
                            contractInterface = new ethers_1.ethers.utils.Interface(contractDeployment.abi);
                            try {
                        ...
                        if (!decodedFunction) {
                            const decoder = new functionSignatureDecoder_1.FunctionSignatureDecoder(hre.ethers.provider);
                            ...
                            if (decoded && decoded.name !== 'Unknown Function') {
                                decodedFunction = {
                                    ...
                                    args: decoded.inputs?.map((input) => input.value) || [],
                ...
                if (receipt.logs && receipt.logs.length > 0) {
                    ...
                    for (const log of receipt.logs) {
                        try {
                            ...
                            const deployments = await hre.deployments.all();
                            const contractDeployment = Object.values(deployments).find(dep => dep.address.toLowerCase() === log.address.toLowerCase());
                            let contractInterface = null;
                            let decoded = null;
                            if (contractDeployment && contractDeployment.abi) {
                                contractInterface = new ethers_1.ethers.utils.Interface(contractDeployment.abi);
                            ...
                            if (!decoded) {
                                ...
                                if (decodedEvent && decodedEvent.name !== 'Unknown Event') {
                                    decoded = {
                                        ...
                                        args: decodedEvent.inputs?.map((input) => input.value) || [],
